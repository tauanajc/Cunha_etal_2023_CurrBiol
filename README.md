...
```bash
sbatch --array=10 ../../scripts/flye.sh PATH_TO_FASTQ_FOLDER
```
...
```bash
sbatch --array=10 PATH_TO_scripts/racon.sh <assembly-file>
...
- In specific assembly folder:
```bash
sbatch --array=8 ../../../../scripts/medaka.sh racon/racon.fasta
```
...
- In specific assembly folder:
```bash
sbatch --array=8 ../../../../scripts/purgedups.sh medaka/consensus.fasta
```
...
```bash
sbatch --array=8 ../../../../scripts/hypo.sh 173m
```
...
Before figuring it out that the problem was snakemake, and not my installation above, I decided to try the Docker container with BlobTools2. Both the full instalation and the docker version work when running the regular blobtool commands, so in the end I am not using the docker. In any case, here are the instructions of how to install and use an example command.
...
```bash
sbatch --array=10 ../../../../scripts/blob-blast.sh
sbatch --array=8 ../../../../scripts/blob-diamond.sh
sbatch --array=8 ../../../../scripts/blob-mapILL.sh
sbatch --array=8 ../../../../scripts/blob-mapONT.sh
sbatch --array=8 ../../../../scripts/blob-busco.sh nematoda_odb10
...
From specific assembly folder, e.g. Nectonema flye folder:
```bash
sbatch --array=8 ../../../../scripts/blobtools.sh taxid
```
...
# run
sbatch --array=1-4 ../../../../scripts/hic-map.sh # array is number of lines in fastq-names-hic.txt
```
...
Salsa scaffolds the assembly using Hi-C data. Salsa uses python 2.<br>
...
#conda create -n salsa -c bioconda salsa2
```

Salsa2 requires a bed file which is converted from bam using Bedtools, an assembly in fasta format, an index for that assembly in .fai (we already made this at the beginning of the mapping pipeline), and a list of restriction enzymes which can be obtained from your library prep kit documentation.<br>
- the Arima Hi-C Library Prep Kit uses the restrcition enzymes GATC and GANTC (from [FAQs](https://arimagenomics.com/faqs/): The Arima-HiC chemistry uses restriction enzymes that digest chromatin at ^GATC and G^ANTC, where N can be any of the 4 genomic bases. Our multiple restriction enzyme chemistry produces the following possible ligation junction motifs: GATCGATC, GANTGATC, GANTANTC, GATCANTC.)

Salsa can also use the assembly graph to assist scaffolding, which supposedly reduces errors, by adding the flag `-g contigs_graph.gfa`. Flye does output this file (`assembly_graph.gfa`), but I am not sure if it is appropriate to use it, since we do several steps after flye and before salsa (polishing, purge dups, decontamination). Read more about this to see if worth it. The -m flag uses Hi-C data to correct input assembly errors, not being used by Shoyo github instructions.
...
#SBATCH -J salsa                                # Job name 
#SBATCH -o salsa.%A.out                         # File to which stdout will be written
#SBATCH -e salsa.%A.err                         # File to which stderr will be written
#SBATCH --mail-type=ALL                         # BEGIN,END,ALL
#SBATCH --mail-user=tauanacunha@g.harvard.edu   # Email address
#SBATCH -N 1                                    # Ensure that all cores are on one machine
#SBATCH -n 1                                    # Number of cores/cpus
#SBATCH -t 02-00:00                             # Runtime in DD-HH:MM
...
LABEL=$(head -n1 hic/fastq-names-hic.txt | rev | cut -d_ -f3- | rev)
SALSA='/n/holylfs04/LABS/giribet_lab/Lab/tauanajc/scripts/SALSA/run_pipeline.py'
GETHIC='/n/holylfs04/LABS/giribet_lab/Lab/tauanajc/scripts/SALSA/convert.sh'
...
# run salsa2
python $SALSA -a blobtools/decontaminated.fasta \
-l hic/mapping/decontaminated.fasta.fai \
-b hic/$LABEL.bed -e GATC,GANTC -o hic/salsa/scaffolds \
-i 50 -m yes
# -m option uses Hi-C data to correct input assembly errors

# create .hic file for visualization
$GETHIC hic/salsa/scaffolds

mv salsa.$SLURM_JOB_ID.* hic/salsa/
```

### Visualize HiC scaffolds

Generate .hic file from SALSA scaffolds

Need to have a [jar file for Juicertools](https://github.com/aidenlab/juicer/wiki/Download) in order to generate the .hic file. Once you download this file on your machine, set the path to the jar file (including the jar file itself) in the SALSA `convert.sh` script to JUICER_JAR variable. Then run as:

```bash
convert.sh hic/salsa/scaffolds
```

Make sure you have latest version of GNU core utils in order to make use of --parallel option in GNU sort. After this script finishes execution, the .hic file will be generated in the SALSA_OUT_DIR as salsa_scaffolds.hic. This file can be loaded to Juicebox and visualized.
...
May 13, 2022 - Danielle and Tim recommended YaHS, apparently giving N50s 2-5x better than salsa.
...
#conda create -n busco -c bioconda -c conda-forge busco=5.1.2
# apparently this version had issue writing wrong output in full_table, which was solved in 5.1.3: https://gitlab.com/ezlab/busco/-/issues/437
...
#SBATCH --mail-type=END                         # BEGIN,END,ALL
#SBATCH --mail-user=tauanacunha@g.harvard.edu   # Email address
#SBATCH -n 16                                   # Number of cores
#SBATCH -N 1                                    # Ensure that all cores are on one machine
#SBATCH -t 0-08:00                              # Runtime in DD-HH:MM
...
busco -i $1 -l $2 -o out_$2 -m genome --augustus --update-data --cpu $SLURM_NTASKS
